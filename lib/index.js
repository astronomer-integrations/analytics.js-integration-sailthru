...
Sailthru.prototype.identify = function(identify) {
  ...
  console.log(integrations);
  var proxy = identify.proxy('integrations');
  console.log('Proxy:');
  console.log(proxy);
  var options = self && self.options;
  var optoutValue = options && options.optoutValue
    || 'none';
  var defaultListName = identify && identify.defaultListName // change how listname is accessed
...
Sailthru.prototype._trackEvents = function(event, track) {
  ...
  var template = props && props.template; // check inside integrations.sailthru
  var payload = null;
  ...
  default:
    ...
    payload = {
      name: event,
      vars: props, // remove personal information from custom events
...
function mapItems(products, productBaseUrl) {
  ...
  if (products) {
    products.forEach(function(product) {
      ...
      var item = {
        // check to see if product values are missing
