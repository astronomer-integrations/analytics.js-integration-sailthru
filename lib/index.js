...
Sailthru.prototype.productAdded = function(track) {
  ...
  var req = { id: email, fields: { purchase_incomplete: 1 } };
  console.log('req:');
  console.log(req);
  ...
  console.log('Req Stringified:');
  console.log(req);
  var sig = md5(auth.secret+auth.apiKey+'json'+req);
  console.log('Sig without encode:');
  console.log(auth.secret+auth.apiKey+'json'+req);
  console.log('Sig:');
  console.log(sig);
  req = encodeURIComponent(req);
  console.log('Req URI Encoded:');
  console.log(req);
